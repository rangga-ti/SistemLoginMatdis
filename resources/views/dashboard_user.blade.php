...
<head>
    ...
    <title>Dashboard User - SistemLoginMatDis</title>
...
<body class="bg-slate-50 dark:bg-[#0B1120] text-slate-900 dark:text-slate-200 antialiased min-h-screen transition-colors duration-300">
    ...
    <header class="sticky top-0 z-50 w-full backdrop-blur-md bg-white/70 dark:bg-[#0B1120]/80 border-b border-slate-200 dark:border-slate-800">
        <div class="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
            ...
            <a href="{{ route('logout.perform') }}"
               ...
               class="inline-flex items-center justify-center bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-200">
                Logout
            </a>
    ...
    <main class="max-w-6xl mx-auto px-6 py-10 space-y-12">
        ...
        <section class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 md:p-8 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative overflow-hidden">
            <div class="absolute top-0 right-0 w-40 h-40 bg-gradient-to-bl from-violet-500/10 to-transparent rounded-bl-full pointer-events-none"></div>
            <div>
                <span class="inline-flex items-center gap-1.5 bg-violet-100 dark:bg-violet-900/40 text-violet-700 dark:text-violet-400 px-3 py-1 rounded-full text-xs font-bold tracking-wider mb-3 uppercase">
                    <span class="w-1.5 h-1.5 rounded-full bg-violet-500 animate-pulse"></span>
                    USER
                </span>
                <h1 class="text-2xl md:text-3xl font-extrabold tracking-tight">Selamat Datang, {{ $user->name }}!</h1>
                <p class="text-slate-500 dark:text-slate-400 mt-1 text-sm md:text-base">Ini adalah halaman utama dengan hak akses standar (User).</p>
            </div>
            
            <div class="w-full md:w-auto bg-slate-50 dark:bg-slate-950 p-4 rounded-2xl border border-slate-200/60 dark:border-slate-800/60 grid grid-cols-1 sm:grid-cols-3 md:flex md:flex-col gap-3 font-mono text-xs text-slate-600 dark:text-slate-400 min-w-[240px]">
                <div><span class="text-slate-400">Badge ID :</span> <span class="text-violet-500 dark:text-violet-400 font-bold">{{ $user->badge_id }}</span></div>
        ...
        <section class="space-y-8">
            <div class="border-b border-slate-200 dark:border-slate-800 pb-4">
                <h2 class="text-xl md:text-2xl font-extrabold tracking-tight">
                    Analisis Logika Sistem Autentikasi
                </h2>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-2xl shadow-sm space-y-4">
                    <h3 class="font-bold text-lg text-slate-800 dark:text-white flex items-center gap-2">
                        <span class="flex items-center justify-center w-6 h-6 rounded-lg bg-violet-100 dark:bg-violet-900/50 text-violet-600 dark:text-violet-400 text-xs font-bold">1</span>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                        <div class="p-3 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200/50 dark:border-slate-800/50">
                            <code class="text-violet-500 dark:text-violet-400 font-bold text-base">A</code>
                            <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Email valid dan terdaftar di database.</p>
                        </div>
                        <div class="p-3 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200/50 dark:border-slate-800/50">
                            <code class="text-violet-500 dark:text-violet-400 font-bold text-base">B</code>
                            <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Password sesuai (lolos <code>Hash::check</code>).</p>
                        </div>
                        <div class="p-3 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200/50 dark:border-slate-800/50">
                            <code class="text-violet-500 dark:text-violet-400 font-bold text-base">C</code>
                            <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Role sesuai dengan record di database.</p>
                        </div>
                        <div class="p-3 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200/50 dark:border-slate-800/50">
                            <code class="text-violet-500 dark:text-violet-400 font-bold text-base">D</code>
                ...
                <div class="flex flex-col gap-6">
                    <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-2xl shadow-sm flex-1 flex flex-col justify-between">
                        ...
                        <div class="my-4 text-center font-mono text-xl md:text-2xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-violet-500 to-purple-500">
                            S = A ∧ B ∧ C ∧ D
                        </div>
                    ...
                    <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-2xl shadow-sm flex-1 flex flex-col justify-between">
                        ...
                        <div class="my-4 text-center font-mono text-xl md:text-2xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-purple-500 to-fuchsia-500">
            ...
            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden">
                <div class="p-6 border-b border-slate-200 dark:border-slate-800">
                    <h3 class="font-bold text-lg text-slate-800 dark:text-white flex items-center gap-2">
                        <span class="flex items-center justify-center w-6 h-6 rounded-lg bg-violet-100 dark:bg-violet-900/50 text-violet-600 dark:text-violet-400 text-xs font-bold">3</span>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-800 text-sm font-mono">
                            ...
                            <tr class="bg-violet-50/40 dark:bg-violet-950/20 hover:bg-violet-50/60 dark:hover:bg-violet-950/30">
            ...
            <div class="bg-[#0F172A] border border-slate-800 rounded-2xl p-6 shadow-xl text-slate-300 font-mono text-xs md:text-sm relative group overflow-hidden">
                <div class="absolute -inset-1 bg-gradient-to-r from-violet-500 to-purple-600 rounded-2xl blur opacity-10 pointer-events-none"></div>
                ...
                <div class="overflow-x-auto text-center md:text-left py-4 px-2 bg-slate-950/60 rounded-xl border border-slate-800/80 shadow-inner">
                    <pre class="inline-block text-left leading-relaxed tracking-wide text-violet-300/90 select-none">
